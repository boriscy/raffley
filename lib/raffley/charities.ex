defmodule Raffley.Charities do
  ...
  def charity_options do
    q = from c in Charity, order_by: [asc: c.name], select: {c.name, c.id}
    Repo.all(q)
  end

  def charity_with_slugs do
    q = from c in Charity, order_by: [asc: c.name], select: {c.name, c.slug}
    Repo.all(q)
  end
